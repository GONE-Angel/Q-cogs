...
class Marry:
    ...
    async def marry(self, ctx, user: discord.Member):
        ...
        if user.mention == requestor:
            ...
            await self.bot.say(embed=em0)
            return
        if ctx.message.author.id in self.data[server.id]["user"] and user.id in self.data[server.id]["user"][ctx.message.author.id]["married_to"] and self.data[server.id]["user"][ctx.message.author.id]["married_to"][user.id]["status"] == "married":
    ...
    async def mlist(self, ctx, user: discord.Member=None):
        ...
        if mlist != "":
            ...
            em.add_field(name='Married to:', value=mlist)
            await self.bot.say(embed=em)
    ...
    @commands.command(pass_context=True, no_pm=True)
    async def marriage(self, ctx, user: discord.Member):
        ...
        if ctx.message.author.id not in self.data[ctx.message.server.id]["user"]:
            msg = "You've never been married."
            em = discord.Embed(description=msg, color=0XF23636)
            await self.bot.say(embed=em)
            return
    ...
    async def _update_data(self, server):
        ...
        for user in self.data[server.id]["user"]:
            if len(user) == 18 and user.isdigit():
                continue
    ...
    async def _divorce(self, server, ctx, user):
        author = ctx.message.author.id
        self.data[server.id]["user"][author]["married_to"][user.id]["history"].append("Divorced: {} {} {}".format(datetime.date.today().day, calendar.month_name[datetime.date.today().month], datetime.date.today().year))
        self.data[server.id]["user"][author]["married_to"][user.id]["status"] = "divorced"
        self.data[server.id]["user"][user.id]["married_to"][author]["history"].append("Divorced: {} {} {}".format(datetime.date.today().day, calendar.month_name[datetime.date.today().month], datetime.date.today().year))
        self.data[server.id]["user"][user.id]["married_to"][author]["status"] = "divorced"
        dataIO.save_json(self.JSON, self.data)

    def new_marriage(self, m_obj):
        default_marriage = {"status":"married", "history":["Married: {} {} {}".format(datetime.date.today().day, calendar.month_name[datetime.date.today().month], datetime.date.today().year)]}
        if not m_obj:
            return default_marriage
        marriage = copy.deepcopy(m_obj)
        marriage["history"].append("Married: {} {} {}".format(datetime.date.today().day, calendar.month_name[datetime.date.today().month], datetime.date.today().year))
    ...
    def _populate_list(self, _list):
        ...
        mlist = ""
        for marriage in _list:
            if _list[marriage]["status"] == "divorced":
                continue
            user = discord.utils.get(self.bot.get_all_members(), id=marriage)
            lastmarriage = _list[marriage]["history"][len(_list[marriage]["history"])-1]
            mlist += "{}, married since {}\n".format(user.display_name, lastmarriage[9:])
        return mlist

    def add_server(self, s_id):
        self.data[s_id] = {}
        self.data[s_id]["user"] = {}
        dataIO.save_json(self.JSON, self.data)
