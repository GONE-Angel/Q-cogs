"""
  ...
  This cog was originally made by https://cogs.red/cogs/Kowlin.
"""
...
class workingantilink:
	...
	def __init__(self, bot):
		...
		self.regex_url = re.compile(r'www.(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+')
		self.regex_url_http = re.compile(r'http[s]?(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+')
		...
		if message.server.id in self.json:
			...
			if self.json[message.server.id]['toggle'] is True:
				...
				if self.json[message.server.id]['strict']:
					for matchurl in self.regex_url.finditer(message.content.lower()):
						asyncio.sleep(0.5)
						...
						break
					for matchhttp in self.regex_url_http.finditer(message.content.lower()):
						asyncio.sleep(0.5)
						await self.bot.delete_message(message)
						if self.json[message.server.id]['dm'] is True:
							await self.bot.send_message(message.author, self.json[message.server.id]['message'])
